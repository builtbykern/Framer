...
- Gallery gap 0 on Work detail; plane cards may cover-crop. Phone Home series rows: 2-col grid, gap 8, cover-crop thumbs, radius 0
- Exactly 3 breakpoints: Desktop 1440 / Tablet 768 / Phone 390
- Home is home-bg. Desktop/Tablet Home = Drift Plane only. Phone Home = one row per Featured Work (native Collection List). Info, Contact, 404, and Work detail are paper + ink
- Motion: Drift Plane (desktop/tablet) + Nav BrandRoll (centered VALE, auto letter-roll loop, no hover, never MENU) + native Settle (Scrim 6px, incoming paper/menu y -32→0, inverse on close/back). No plus, no Close word, no hamburger, no X. No Page Effect Fade. No Layout Templates. No Custom Code frost. Do not insert LetterRollMenu. Paper page *content* has no ken burns, scroll-scrub, or gallery stagger. English, quiet visual-director voice. No lorem ipsum
...
component (centered BrandRoll: one word VALE, auto-roll loop), a native Scrim + PageSurface/MenuSurface settle (small blur, bg lowers), and on Phone Home a Collection List of Work (one row per series).

DO NOT:
- create Index, Privacy, Journal, blog, shop, or any route beyond Home, /info, /contact, 404, and CMS Work detail /work/{slug}
- add video, lightbox, overlay *project viewer*, or a second hero on Desktop/Tablet Home. Desktop/Tablet Home is only the Drift Plane (+ Nav + hint). Phone Home is series rows (not the Plane). Nav bar is only BrandRoll, centered: one word VALE with an auto letter-roll loop (no hover). Variant open is a full-viewport *visual* menu (33/67 paper + still). A Scrim (paper 16% + 6px background blur) plus lowering surfaces on page/menu change is required — it is not a project viewer and not a Gregor 12px veil. Do not insert LetterRollMenu (multi-row). Do not add a plus, Close label, MENU label, hamburger, or X
- use Unsplash or any stock plugin except Lummi, and only when the phase says so
- use Fable 5 or GPT 5.6 Sol
- write custom code. Do not edit Site Settings Custom Code. Do not add a Page Effect Fade (Safari skips it; Settle is native layers). Do not add plane|snap code — Phone Home is native rows
