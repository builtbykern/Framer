...
// Remove prior instances if re-run
...
function walk(n) {
  if (!n) return
  if (/TerritoryHoverMedia/i.test(n.name || "") || String(n.component || "").includes(codeId)) {
    toDel.push(n.id)
  }
...
const cmds = []
for (const id of toDel) cmds.push(`DEL ${id}`)
...
for (const s of slots) {
  ...
  cmds.push(
    `SET ${s.photo} width="1fr" height="${s.h}" minHeight="${s.h}" maxHeight="${s.h}" aspectRatio=1.6 overflow="clip" fill="transparent" backgroundColor="transparent"`
  )
  cmds.push(
  ...
  cmds.push(
    `+ComponentInstanceNode ${s.tag} parent="${s.photo}" component="codeFile/${codeId}:default" name="Arbour_TerritoryHoverMedia" position="absolute" top="0px" left="0px" width="100%" height="100%" $control__image=${HERO} $control__imageB=${MAP} $control__intervalMs=900 $control__zoom=1.04 $control__showView=true $control__viewLabel="VIEW →" $control__accent=${CHARTREUSE}`
  )
}
