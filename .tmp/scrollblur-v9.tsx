...
/**
 * While scroll-animating, soft-cap blur paint cost.
 * Always On / static preview still use the full Blur control (≤20).
 */
const ANIMATED_BLUR_CAP = 14

/** Below this strength while dissolving, drop backdrop blur and finish on opacity. */
const BLUR_CUTOFF = 0.32
...
export default function BuiltByKern_ScrollBlur(
    ...
    const blurLayersRef = useRef<HTMLElement[]>([])
    ...
    // Scroll path: cheaper paint. Canvas Always On keeps the full requested blur.
    const runtimeBlur = shouldAnimateByScroll
        ? Math.min(safeBlur, ANIMATED_BLUR_CAP)
        : safeBlur
    ...
    useSafeLayoutEffect(() => {
        ...
        blurLayersRef.current = Array.from(
            node.querySelectorAll<HTMLElement>("[data-kern-blur]")
        )

        const write = (value: number) => {
            const s = Math.max(0, value)
            const dissolving = targetStrengthRef.current === 0
            // Exit: cut backdrop paint early; finish the dissolve on opacity (composite).
            const blurS = dissolving && s < BLUR_CUTOFF ? 0 : s
            const opacity =
                dissolving && s < BLUR_CUTOFF
                    ? Math.max(0, s / BLUR_CUTOFF)
                    : s <= 0.001
                      ? 0
                      : 1
            node.style.opacity = String(opacity)

            const layers = blurLayersRef.current
            for (let i = 0; i < layers.length; i += 1) {
                const el = layers[i]
                const base = Number(el.dataset.kernBlur)
                if (!Number.isFinite(base)) continue
                const px = Math.max(0, base * blurS)
    ...
    useEffect(() => {
        ...
        // One activity pulse per frame — scroll fires far more often than paint.
        const onScroll = () => {
        ...
        // Prefer scrollend when available — cleaner idle edge after momentum.
