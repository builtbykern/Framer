...
if (!driftSrc.includes("function tileScreenXY")) {
    throw new Error("tileScreenXY missing")
}
if (driftSrc.includes("calc(50% - ${opts.tileW / 2}")) {
    throw new Error("divergent freeze pose still present")
}
...
for (const [id, name] of [
    ["WQLkyLRf1", "desktop-canvas.png"],
    ["BjqrvIntT", "tablet-canvas.png"],
    ["nyI5jW7lA", "phone-canvas.png"],
]) {
    try {
    ...
    JSON.stringify(
        {
            ...
            stillsTc,
            driftTc,
            samePose: String(driftLive.content).includes("tileScreenXY"),
            errors: keep.errors,
            shots,
        },
        null,
