...
if (!driftSrc.includes('overflowY: "visible"')) {
    throw new Error("collection overflowY not visible")
}
...
try {
    ...
    fs.writeFileSync(path.join(out, "collection-uncrop.png"), r.data)
    shot = r.data.length
} catch (e) {
    shot = String(e)
}
    ...
    JSON.stringify(
        {
            ...
            stillsTc,
            driftTc,
            visible: String(driftLive.content).includes('overflowY: "visible"'),
            errors: keep.errors,
            shot,
        },
        null,
