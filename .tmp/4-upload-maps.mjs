...
const ASSETS = "/Users/noel/.cursor/projects/Users-noel-Desktop-Framer/assets";
...
let mapField = fields.find((f) => f.name === "Map" || f.name === "Territory Map");

if (!mapField) {
  const created = await col.addFields([{ type: "image", name: "Map" }]);
  mapField = created[0];
  console.log("created Map field", mapField.id);
} else {
  console.log("existing Map field", mapField.id, mapField.name);
}
...
for (const m of maps) {
  ...
  console.log("uploaded", m.slug, asset.url?.slice(0, 80));
}
...
console.log("mapFieldId", mapField.id);
