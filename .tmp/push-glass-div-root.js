...
console.log("home", JSON.stringify(home).slice(0, 400))
...
console.log("thumb", JSON.stringify(thumb).slice(0, 400))
...
const keys = Object.keys(h.attributes || {}).filter((k) =>
    /control|action|link|content/i.test(k)
)
console.log(
    "controls",
    JSON.stringify({
        keys,
        content: String(h.attributes?.["$control__content"] ?? "").slice(0, 500),
        action: h.attributes?.["$control__action"],
    })
)
...
console.log("shot", shot.data.length, homeId, thumb.renamedIds?.glassThumb)
